...
namespace SETUtil.Extend
{
	public static class ExtendUtil
	{
		...
		{
			// handle int, it's special
			var _destinationType = typeof(T);
			int _intCast;
			if (_destinationType == typeof(int) && int.TryParse(obj.ToString(), out _intCast)) {
				result = (T)(object)_intCast;
				return true;
			}

			// handle generic things
			if (obj is T) {
				result = (T) obj;
				return true;
			}
		...
		public static void Debug(this U.Transform[] tArr)
		{
			...
			_log.AppendLine("Transform Array Contents:");
			
			foreach(U.Transform t in tArr){
				_log.AppendLine(t.name);
			}
			...
			if (_pathArray.Length > 1) {
				...
				for (int i = 0; i < _pathArray.Length - 1 /*go only through the parents, hence -1*/; i++) {
					if (_so_parentDir != null) {
						...
						U.Debug.LogError("[SETUtil_EditorUtil.FindDeepProperty ERROR] Invalid path element: " + _so_parentDir);
				}
			} else if (_pathArray.Length == 1) {
				_so_targ = so.FindProperty(_pathArray[0]);
			} else {
				U.Debug.LogError("[SETUtil_EditorUtil.FindDeepProperty ERROR] Empty target path!");
				return null;
			}

			if (_so_targ == null) {
				U.Debug.LogError("[SETUtil_EditorUtil.FindDeepProperty ERROR] Nonexistent or inaccessible property: " +
				                 path + ". Make sure you are not targeting a MonoBehavior-derived class instance.");
				return null;
			}
